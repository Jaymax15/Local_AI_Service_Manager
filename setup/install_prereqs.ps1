...
function Step-Start {
    ...
    Write-Host ""
    Write-Host ("STEP {0}/{1}: {2}" -f $Script:CurrentStep, $Script:TotalSteps, $Name) -ForegroundColor Cyan
...
function Get-InstalledWslDistros {
    try {
        # WSL output can contain null characters depending on console/encoding.
        $raw = (& wsl.exe -l -q 2>$null) -join "`n"
        $clean = ($raw -replace "`0", "").Trim()
        if ([string]::IsNullOrWhiteSpace($clean)) {
            return @()
        }
        return $clean -split "`n" | ForEach-Object { $_.Trim() } | Where-Object { $_ }
    } catch {
        return @()
    }
}
...
function Ensure-UbuntuWsl {
    ...
    if ($distros | Where-Object { $_ -match "^Ubuntu" }) {
        Write-Log "Ubuntu WSL distro appears to be installed: $($distros -join ', ')" "OK" "Green"
        return
    }

    Write-Log "Ubuntu WSL distro not found. Installing Ubuntu..." "INFO" "Yellow"
    $output = & wsl.exe --install -d Ubuntu --no-launch 2>&1
    $code = $LASTEXITCODE
    $outputText = ($output -join "`n")
    Add-Content -Path $LogFile -Value $outputText -Encoding UTF8

    if ($code -eq 0) {
        Write-Log "Ubuntu install requested successfully." "OK" "Green"
    ...
    if ($outputText -match "ERROR_ALREADY_EXISTS" -or $outputText -match "already exists") {
        Write-Log "Ubuntu already exists. Continuing." "OK" "Green"
        return
    }

    Add-Error "ERROR[WSL-UBUNTU-$code]: Ubuntu install failed. $outputText"
}
...
function Install-PythonBestAvailable {
    ...
    foreach ($id in $pythonIds) {
        ...
        $null = & winget show --id $id -e --accept-source-agreements 2>&1
