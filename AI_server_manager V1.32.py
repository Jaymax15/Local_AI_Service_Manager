...
class AIManager:
    def __init__(self, root):
        self.root = root
        self.root.title("AI Server Manager V32")
    ...
    def show_loading_screen(self):
        ...
        bar_outer = tk.Frame(box, bg="#222222", height=24, highlightbackground="#303030", highlightthickness=1)
        bar_outer.pack(fill=tk.X, padx=70)
        bar_outer.pack_propagate(False)
        self.loading_bar_outer = bar_outer
        self.loading_bar = tk.Frame(bar_outer, bg="#1f7a3a", width=1)
        self.loading_bar.pack(side=tk.LEFT, fill=tk.Y)
    ...
    def update_loading(self, percent, message):
        def apply():
            try:
                ...
                width = max(1, int(self.loading_bar_outer.winfo_width() * pct / 100))
                self.loading_bar.config(width=width)
                self.loading_overlay.lift()
            except Exception:
                pass
        self.safe_ui(apply)
    ...
    def preload_startup_data(self):
        ...
        steps = [
            ...
            (22, "Checking installed services...", lambda: self.refresh_installed_services(log=False)),
            (42, "Preparing AI service monitor...", self._preload_service_states),
    ...
    def make_ui(self):
        ...
        uihelpers.rounded_button(top, "START ALL", command=self.start_all_threaded, bg="#1f7a3a", width=170, height=38).pack(side=tk.LEFT, padx=5)
        uihelpers.rounded_button(top, "STOP ALL", command=self.stop_all_threaded, bg="#9b1c1c", width=170, height=38).pack(side=tk.LEFT, padx=5)
        uihelpers.rounded_button(top, "SETTINGS", command=self.open_settings_panel, bg="#234f9c", width=190, height=38).pack(side=tk.LEFT, padx=5)
        uihelpers.rounded_button(top, "SERVICES", command=self.open_services_panel, bg="#5b357a", width=170, height=38).pack(side=tk.LEFT, padx=5)
    ...
    def has_docker_access(self):
        return service_ok("command -v docker >/dev/null 2>&1 && (docker info >/dev/null 2>&1 || sudo -n docker info >/dev/null 2>&1)")
    ...
    def install_sudo_permissions_from_settings(self, sudo_password, on_status=None):
        """Install /etc/sudoers.d/ai-manager from Settings.

        The password is used once with sudo -S. It is not saved.
        This writes a validated sudoers include file so the manager can run
        systemctl/pkill/docker/journalctl without asking again.
        """
        sudo_password = sudo_password or ""

        def status(message, color="#cccccc"):
            self.write(f"[SETTINGS] {message}")
            if on_status is not None:
                self.safe_ui(lambda m=message, c=color: on_status(m, c))

        if not sudo_password.strip():
            status("Enter your WSL/Linux sudo password first.", "#ff4444")
            return
...
$USER_NAME ALL=(root) NOPASSWD: $SYSTEMCTL
$USER_NAME ALL=(root) NOPASSWD: $SYSTEMCTL *
$USER_NAME ALL=(root) NOPASSWD: $PKILL
$USER_NAME ALL=(root) NOPASSWD: $PKILL *
$USER_NAME ALL=(root) NOPASSWD: $JOURNALCTL
$USER_NAME ALL=(root) NOPASSWD: $JOURNALCTL *
EOF

if [ -n "$DOCKER" ]; then
  cat >> "$TMP" <<EOF
$USER_NAME ALL=(root) NOPASSWD: $DOCKER
$USER_NAME ALL=(root) NOPASSWD: $DOCKER *
EOF
fi
...
echo "Testing passwordless manager commands..."
...
if sudo -n pkill --version >/dev/null 2>&1; then
  echo "pkill sudo: OK"
else
...
if [ -n "$DOCKER" ]; then
  if sudo -n docker version >/dev/null 2>&1; then
    ...
    echo "docker sudo: WARNING - Docker may not be running, but sudoers entry was written."
  fi
fi
echo "Sudo setup complete. Start/stop commands can now use sudo -n."
'''

        def worker():
            status("Installing sudo access. Entered password is not saved.", "#ffaa00")
            script_path = "/tmp/ai-manager-install-sudo.sh"
            try:
                write_proc = subprocess.run(
                    ["wsl", "bash", "-lc", f"cat > {script_path!r} && chmod 700 {script_path!r}"],
                    input=installer_script.replace("\r\n", "\n").replace("\r", "\n"),
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                    text=True,
                    encoding="utf-8",
                ...
                if write_proc.returncode != 0:
                    self.write("[SETTINGS/SUDO] Could not write installer script.")
                    self.write(write_proc.stdout or "")
                    status("Sudo setup failed before install.", "#ff4444")
                    return

                proc = subprocess.Popen(
                    ["wsl", "bash", "-lc", f"sudo -k; sudo -S -p '' bash {script_path!r}; rc=$?; rm -f {script_path!r}; exit $rc"],
                ...
                for line in output.splitlines():
                    if line.strip():
                        self.write(f"[SETTINGS/SUDO] {line}")

                if proc.returncode == 0 and has_sudo_access():
                    status("Sudo access installed successfully. Try START ALL again.", "#00ff99")
                elif proc.returncode == 1:
                    status("Sudo setup failed. Wrong password or sudo denied.", "#ff4444")
                else:
                    status(f"Sudo setup failed with code {proc.returncode}. Check terminal output.", "#ff4444")
            except subprocess.TimeoutExpired:
                status("Sudo setup timed out. Check WSL/sudo prompt state.", "#ff4444")
            except Exception as e:
                status(f"Sudo setup error: {e}", "#ff4444")
...
"""
    ...
    def wait_for_service_start(self, key):
        ...
        while time.time() - start < timeout:
            ...
            if not self.service_process_active(key):
                # Give short-lived successful start commands a moment to expose their API.
                time.sleep(0.5)
            time.sleep(0.5)
    ...
    def silly_running(self):
        return service_ok("curl -fsS http://127.0.0.1:8000 >/dev/null 2>&1") or service_ok(
    ...
    def wait_until_ready(self):
        ...
        for _ in range(90):
            ...
            if wanted_ready:
                ...
                self.write("SillyTavern: http://localhost:8000", "endpoint")
                self.write("Ollama API: http://127.0.0.1:11434", "endpoint")
                self.write("XTTS2 API: http://127.0.0.1:8020", "endpoint")
                return
            time.sleep(0.5)
...
roots=( {roots_text} )
printf 'ollama=%s\n' "$( (command -v ollama >/dev/null 2>&1 || systemctl list-unit-files ollama.service >/dev/null 2>&1 || found_dir ollama ollama "${{roots[@]}}") && echo 1 || echo 0 )"
...
"""
    ...
    def service_loop(self):
        while True:
            try:
                ...
                snapshot = str(states) + str(active) + str(self.healthy_since) + str(uptime_tick) + str(self.running) + str(self.stopping) + str(self.service_enabled)
    ...
    def render_services(self, mapping, states, active=None):
        ...
        if not rows:
            self.services.insert(tk.END, "No enabled installed services. Open Services or Add service.\n", "stopped")
