...
SETTINGS_FILE = paths.DATA_ROOT / "settings.txt"
LEGACY_SETTINGS_FILE = paths.COMPONENTS_ROOT / "settings.txt"
...
def load_settings():
    try:
        source_file = SETTINGS_FILE if SETTINGS_FILE.exists() else LEGACY_SETTINGS_FILE
        data = json.loads(source_file.read_text(encoding="utf-8")) if source_file.exists() else {}
    except Exception:
        data = {}
...
def build_settings(parent, manager):
    ...
    def run_updates():
        ...
        def worker():
            app_changed = False
            try:
                from components.ui import models as model_catalog
                from components.update import updateutil
