...
from components.ui import settings as settings_store
from components.ui import uihelpers
from components.core import paths


MODEL_LIST_PATH = paths.DATA_ROOT / "models.txt"
LEGACY_MODEL_LIST_PATH = paths.COMPONENTS_ROOT / "models.txt"
MODEL_LIST_URL = "https://raw.githubusercontent.com/Jaymax15/Local_AI_Service_Manager/main/components/data/models.txt"
...
def _ollama_portable_command(extra_command=""):
    ...
    """
    ai_dir = _to_wsl_path(paths.PROJECT_ROOT).rstrip('/')
...
def _ensure_model_file():
    try:
        ...
        if (not MODEL_LIST_PATH.exists()) and LEGACY_MODEL_LIST_PATH.exists():
            legacy_text = LEGACY_MODEL_LIST_PATH.read_text(encoding="utf-8", errors="ignore")
            if legacy_text.strip() and "Compatibility note" not in legacy_text:
                MODEL_LIST_PATH.write_text(legacy_text, encoding="utf-8")
