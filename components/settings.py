...
def processor_options(manager=None, supports_gpu=True):
    opts = [("Auto", "auto"), (cpu_label(manager), "cpu")]
    if supports_gpu:
        rows = []
        try:
            rows = getattr(manager, "gpu_rows_cache", []) or []
        except Exception:
            rows = []
        if not rows:
            try:
                ...
                ], text=True, encoding="utf-8", errors="replace", timeout=1.5).strip()
                for line in out.splitlines():
                    parts = [p.strip() for p in line.split(",", 1)]
                    if parts and parts[0].isdigit():
                        rows.append((parts[0], parts[1] if len(parts) > 1 else f"GPU {parts[0]}"))
            except Exception:
                rows = []
        for row in rows:
            try:
                idx, name = str(row[0]), str(row[1])
                opts.append((f"GPU {idx} - {name}", f"gpu:{idx}"))
            except Exception:
                pass
    return opts
...
def build_settings(parent, manager):
    ...
    update_log_line("Ready. Click Update to check GitHub for a newer models.txt file.", "muted")

    def run_models_update():
        ...
        update_log_line("Starting model-list update...", "info")

        def worker():
            try:
                from components import models as model_catalog

                def log_from_worker(text):
                    try:
                        manager.safe_ui(lambda t=text: update_log_line(t))
                    except Exception:
                        update_log_line(text)

                result = model_catalog.update_model_catalog_from_github(log_callback=log_from_worker)
                if result.get("ok"):
                    try:
                        manager.write("[SETTINGS] Model list update check complete.", "system")
                    except Exception:
                        pass
            except Exception as e:
                try:
                    manager.safe_ui(lambda err=e: update_log_line(f"Download unsuccessful: {err}", "error"))
                except Exception:
                    pass
            finally:
                def finish():
    ...
    update_btn = uihelpers.rounded_button(update_button_holder, "Update", command=run_models_update, bg=colors["button_primary"], width=138, height=34, font=("Segoe UI", 9, "bold"))
    update_btn.pack(side=tk.RIGHT)
    tk.Label(update_panel, text="Updates the curated Ollama model list used by Manage Models.", bg=colors["panel_bg"], fg=colors["muted_fg"], font=("Segoe UI", 9), anchor="w").pack(fill=tk.X, pady=(8, 0))
