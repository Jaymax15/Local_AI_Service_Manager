...
# components.py now lives in AI_SERVER/components/core. Use shared path helper.
LOCAL_AI_DIR = paths.PROJECT_ROOT
...
def start_ollama(manager):
    processor_script = "\nOLLAMA_BASE=" + _q(AI_DIR + "/ollama") + "\n" + processorhandler.ollama_processor_repair_script(manager)
    return manager.run_logged("OLLAMA", processor_script + OLLAMA_RUN)
...
def start_xtts(manager):
    processor_script = processorhandler.docker_compose_processor_repair_script(manager, "xtts", "XTTS", ["xtts"])
    proc = manager.run_logged("XTTS", _xtts_compose_repair_prefix() + processor_script + _docker_compose_up_command("XTTS2", "XTTS", XTTS_DIR))
    if proc is None:
...
def start_kokoro(manager):
    processor_script = processorhandler.docker_compose_processor_repair_script(manager, "kokoro", "KOKORO", ["kokoro-fastapi", "kokoro"])
    return manager.run_logged("KOKORO", f"cd {_q(KOKORO_DIR)} || {{ echo \"[KOKORO] ERROR[20]: folder missing: {KOKORO_DIR}\"; exit 20; }}\n" + processor_script + KOKORO_RUN)
...
echo "[SILLYTAVERN] V79 uses Docker host networking so SillyTavern can reach Ollama at 127.0.0.1:11434."
...
def start_openwebui(manager):
    repair = f"""
cd {_q(OPEN_WEBUI_DIR)} || {{ echo "[OPENWEBUI] ERROR[20]: folder missing: {OPEN_WEBUI_DIR}"; exit 20; }}
if [ -f docker-compose.yml ] && grep -q "restart: unless-stopped" docker-compose.yml 2>/dev/null; then
  echo "[OPENWEBUI] DEBUG: disabling Docker autostart for Open WebUI."
  python3 - <<'PY'
from pathlib import Path
p = Path('docker-compose.yml')
s = p.read_text()
s = s.replace('restart: unless-stopped', 'restart: "no"')
p.write_text(s)
PY
fi
"""
    return manager.run_logged("OPENWEBUI", repair + OPEN_WEBUI_RUN)
