...
def _ollama_portable_repair_prefix():
    ...
    V87 avoids fragile heredoc/nested-shell quoting.  The previous V86 prefix
    could break inside bash and leak error text into the Model Manager list.
    This version writes the override with printf, checks the live process env
    with sudo cat, and refuses to continue if Ollama is not using the portable
    model folder.
    """
    ai_dir = _to_wsl_path(Path(__file__).resolve().parent.parent).rstrip('/')
    base = f"{ai_dir}/ollama"
    models = f"{base}/models"
    # Paths are generated by the manager and should be /mnt/<drive>/... here.
    # Quote defensively anyway, without depending on shlex for older embedded builds.
    base_q = "'" + base.replace("'", "'\\''") + "'"
    models_q = "'" + models.replace("'", "'\\''") + "'"
    return (
        f"OLLAMA_BASE={base_q}; OLLAMA_MODEL_DIR={models_q}; "
        ...
        ">/tmp/ai-manager-models-ollama-override.conf; "
        ...
        "if ! check_ollama_env; then echo 'ERROR[MODELS-PATH-001]: Ollama is not using portable model folder.'; exit 86; fi; "
    )
